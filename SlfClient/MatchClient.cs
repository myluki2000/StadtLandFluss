...
namespace SlfClient
{
    internal class MatchClient
    {
        ...
        /// </summary>
        public event EventHandler? OnRoundResults;
        ...
        private void ReceiveNetworkingMessages()
        {
            ...
            else if(packet is RoundResultPacket roundResultPacket)
            {
                ...
                for(int i = 0; i < playerCount; i++)
                {
                    ...
                    round.PlayerAnswers.Add(playerId, new MatchRound.Answers(city, country, river));
                }

                FinishedRounds.Add(round);

                OnRoundResults?.Invoke(this, EventArgs.Empty);
            }
            else
            {
